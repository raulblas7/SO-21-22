...
int init_module(void)
{
    ...
    set_leds(get_kbd_driver_handler(), ALL_LEDS_OFF);
...
device_write(struct file *filp, const char *buff, size_t len, loff_t *off)
{
    ...
    int err = 0;
    //Apagamos las leds
    unsigned int stateLEDS = ALL_LEDS_OFF;
    //copiar de forma segura los bytes de buff a un array auxiliar
    if (copy_from_user(kbuf, buff, len))
        return -EFAULT;

    //Se ha de tener en cuenta que el caracter terminador (’\0’) NO está incluido al final del array
    kbuf[len] = '\0';

    //recomerremos el array auxiliar para poner las masks
    while (kbuf[i] != '\n' && i < len)
    {
        switch (kbuf[i])
        {
    ...
    if ((err = set_leds(kbd_driver, stateLEDS)))
        return err;
